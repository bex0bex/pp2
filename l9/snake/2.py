...
# Window size and block size for grid cells
...
class Food:
    def __init__(self):
        self.color = RED
        self.weight = random.randint(1, 3)  # Assign random weight to the food (1 to 3)
        self.spawn()
        self.spawn_time = pygame.time.get_ticks()  # Time when food is spawned
    ...
    def is_expired(self):  # Check if food has expired
        current_time = pygame.time.get_ticks()
        if current_time - self.spawn_time > 5000:  # 5000 milliseconds = 5 seconds
            return True
        return False

    def reset(self):  # Reset food (re-spawn)
        self.spawn()
...
while True:
    ...
    # Check if food has expired and reset it if needed
    if food.is_expired():
        food.reset()
